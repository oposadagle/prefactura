...
CREATE UNIQUE INDEX IF NOT EXISTS idx_masas_mat_unique ON masas_mat("AÑO", "MES", "CLIENTE", "REGIONAL");
...
-- Vista materializada para utiles
DROP MATERIALIZED VIEW IF EXISTS utiles_mat;
CREATE MATERIALIZED VIEW utiles_mat AS
SELECT * FROM utiles;

CREATE UNIQUE INDEX IF NOT EXISTS idx_utiles_mat_unique ON utiles_mat(id);

...
class SolicitudController extends Controller
{
        ...
        try {
            ...
            foreach ($peticiones as $p) {
                ...
                if ($p->pago_completo === 'SI') {
                    $mensaje = "Estimado proveedor, GLE Colombia SAS informa que inicio un proceso de pago por concepto del anticipo correspondiente al manifiesto: {$p->razon}\n\n".
                               "RESUMEN\n".
                               "reteica: {$val_reteica}\n".
    ...
    public function pagarSaldos(Request $request)
    {
        ...
        try {
            ...
            foreach ($peticiones as $p) {
                ...
                $mensaje = "Estimado proveedor, GLE Colombia SAS informa que inicio un proceso de pago por concepto del saldo correspondiente al manifiesto: {$p->razon}\n\n".
                           "RESUMEN:\n".
                           "COSTO: {$val_costo}\n".
    ...
    public function pagarCuentas(Request $request)
    {
        ...
        try {
            ...
            foreach ($peticiones as $p) {
                ...
                $mensaje = "Estimado proveedor, GLE Colombia SAS informa que inicio un proceso de pago por concepto de la cuenta de cobro relacionado al manifiesto: {$p->razon}\n\n".
                           "RESUMEN\n".
    ...
    public function adelanto(Request $request)
    {
        ...
        $callback = function () use ($incluidos, $excluidos, $festivos, $year, $month) {
            ...
            fputcsv($file, [
                ...
                'TIPO PAGO', 'FECHA ENVIO CUMPLIDO', 'FECHA TENTATIVA', 'ENVIADO', 'NOTAS DEDUCCIONES',
            ], ';');
    ...
    public function update5(Request $request, $id)
    {
        ...
        // Validación: Tiempo transcurrido desde fecha_descargue (REGLA ELIMINADA DEL CIERRE)
        /* if ($restricciones->fecha_descargue) {
            ...
            if ($diasPasados < $diasRequeridos) {
                return back()->with('error', "No se puede finalizar el servicio. Deben pasar al menos {$diasRequeridos} días calendario desde la fecha de descargue ({$restricciones->fecha_descargue}).");
            }
        } else {

...
return new class extends Migration
{
    ...
    {
        // Vista materializada masas_mat
        DB::statement('DROP MATERIALIZED VIEW IF EXISTS masas_mat');
...
SQL;
        ...
        DB::statement($createMasas);
        DB::statement('CREATE UNIQUE INDEX IF NOT EXISTS idx_masas_mat_unique ON masas_mat("AÑO", "MES", "CLIENTE", "REGIONAL")');

        // Vista materializada utiles_mat (solo si existe la tabla utiles)
        $exists = DB::select("SELECT 1 FROM information_schema.tables WHERE table_name = 'utiles'");
        if (!empty($exists)) {
            DB::statement('DROP MATERIALIZED VIEW IF EXISTS utiles_mat');
            DB::statement('CREATE MATERIALIZED VIEW utiles_mat AS SELECT * FROM utiles');
            DB::statement('CREATE UNIQUE INDEX IF NOT EXISTS idx_utiles_mat_unique ON utiles_mat(id)');
        }
    }

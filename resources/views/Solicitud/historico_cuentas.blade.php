                        ...
                        <tbody style="font-size: 12px;font-family: Titillium Web;">
                            @foreach ($diarias as $diario)
                                <tr style="text-align: center">
                                    ...
                                    @php
                                        $base = floatval($diario->cargaone ?? 0) + floatval($diario->cargatwo ?? 0) + floatval($diario->standby ?? 0) + floatval($diario->costo_desplazamiento ?? 0);
                                        $reteica = ($diario->ica == 'SI') ? ($base * 0.00414) : 0;
                                        $retefuente = $base * 0.01;
                                        $total = $base - ($reteica + $retefuente);
                                    @endphp
                                    <td class="celdas" style="border: 1px solid #9FAACC;padding-top:10px;padding-bottom:10px;">{{ number_format($reteica, 0, ',', '.') }}</td>
                                    <td class="celdas" style="border: 1px solid #9FAACC;padding-top:10px;padding-bottom:10px;">{{ number_format($retefuente, 0, ',', '.') }}</td>
                                    <td class="celdas" style="border: 1px solid #9FAACC;padding-top:10px;padding-bottom:10px;font-weight: bold;">{{ number_format($total, 0, ',', '.') }}</td>

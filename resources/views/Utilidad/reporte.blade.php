<x-header />

<div class="card">
    <div class="card-header">
        <div class="card-header d-flex justify-content-between align-items-center m-2">
            <div class="d-flex">
                <svg fill="#000000" width="24" height="24" viewBox="0 0 24 24" id="download-alt-3"
                    data-name="Flat Color" xmlns="http://www.w3.org/2000/svg" class="icon flat-color">
                    <g id="SVGRepo_bgCarrier" stroke-width="0" />
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" />
                    <g id="SVGRepo_iconCarrier">
                        <path id="secondary"
                            d="M15.71,7.29a1,1,0,0,0-1.42,0L13,8.59V3a1,1,0,0,0-2,0V8.59L9.71,7.29A1,1,0,1,0,8.29,8.71l3,3a1,1,0,0,0,1.42,0l3-3A1,1,0,0,0,15.71,7.29Z"
                            style="fill: #fe252e;" />
                        <path id="primary"
                            d="M22,12.68,19.18,4.37A2,2,0,0,0,17.28,3H16a1,1,0,0,0,0,2h1.28l2.33,7H16a1,1,0,0,0-1,1,3,3,0,0,1-6,0,1,1,0,0,0-1-1H4.39L6.72,5H8A1,1,0,0,0,8,3H6.72a2,2,0,0,0-1.9,1.37L2.05,12.68A1.19,1.19,0,0,0,2,13v7a2,2,0,0,0,2,2H20a2,2,0,0,0,2-2V13A1.19,1.19,0,0,0,22,12.68Z"
                            style="fill: #5f6379;" />
                    </g>
                </svg>
                <h4 class="card-title" style="margin-left: 10px;">DESCARGAR REPORTE DE MASIVOS</h4>
            </div>
        </div>

        <form method="POST" action="{{ route('utilidad.reporte') }}">
            @csrf

            <!--container 1-->
            <div class="container">
                <div class="row gx-1 justify-content-between d-flex py-4">
                    <div class="col-lg-12 col-md-12 ">
                        <div class="form-floating mb-3">
                            <input type="date" class="form-control" id="floatingInput" name="fecha_inicio">
                            <label for="floatingInput">Fecha de inicio</label>
                        </div>

                        <div class="form-floating mb-3">
                            <input type="date" class="form-control" id="floatingInput" name="fecha_fin">
                            <label for="floatingInput">Fecha final</label>
                        </div>

                        <div class="form-floating mb-3">
                            <select name="cliente" class="form-select" id="floatingSelect">
                                <option value="">Seleccione un cliente</option>
                                @foreach ($clientes as $cliente)
                                    <option value="{{ $cliente }}">{{ $cliente }}</option>
                                @endforeach
                            </select>
                            <label for="floatingSelect">Cliente</label>
                        </div>

                        <div class="form-floating mb-3">
                            <select name="congelado" class="form-select" id="floatingSelect">
                                <option value="">Seleccione una opción</option>
                                @foreach ($congelados as $congelado)
                                    <option value="{{ $congelado }}">{{ $congelado }}</option>
                                @endforeach
                            </select>
                            <label for="congelado">Congelado</label>
                        </div>

                        <div class="form-floating mb-3">
                            <select name="facturado" class="form-select" id="floatingSelect">
                                <option value="">Seleccione una opción</option>
                                @foreach ($facturados as $facturado)
                                    <option value="{{ $facturado }}">{{ $facturado }}</option>
                                @endforeach
                            </select>
                            <label for="facturado">Facturado</label>
                        </div>
                    </div>
                    {{-- <button type="submit" class="btn btn-secondary py-2 my-2">Mostrar datos</button>                     --}}
                    <button type="submit" name="descargar" value="1" class="btn btn-secondary py-3" style="background-color: #5f6379">
                        <span>
                            <svg style="margin-right: 6px;" width="16" height="16" viewBox="0 0 32 32"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <defs>
                                    <linearGradient id="a" x1="4.494" y1="-2092.086" x2="13.832"
                                        y2="-2075.914" gradientTransform="translate(0 2100)"
                                        gradientUnits="userSpaceOnUse">
                                        <stop offset="0" stop-color="#18884f" />
                                        <stop offset="0.5" stop-color="#117e43" />
                                        <stop offset="1" stop-color="#0b6631" />
                                    </linearGradient>
                                </defs>
                                <title>file_type_excel</title>
                                <path
                                    d="M19.581,15.35,8.512,13.4V27.809A1.192,1.192,0,0,0,9.705,29h19.1A1.192,1.192,0,0,0,30,27.809h0V22.5Z"
                                    style="fill:#185c37" />
                                <path
                                    d="M19.581,3H9.705A1.192,1.192,0,0,0,8.512,4.191h0V9.5L19.581,16l5.861,1.95L30,16V9.5Z"
                                    style="fill:#21a366" />
                                <path d="M8.512,9.5H19.581V16H8.512Z" style="fill:#107c41" />
                                <path
                                    d="M16.434,8.2H8.512V24.45h7.922a1.2,1.2,0,0,0,1.194-1.191V9.391A1.2,1.2,0,0,0,16.434,8.2Z"
                                    style="opacity:0.10000000149011612;isolation:isolate" />
                                <path
                                    d="M15.783,8.85H8.512V25.1h7.271a1.2,1.2,0,0,0,1.194-1.191V10.041A1.2,1.2,0,0,0,15.783,8.85Z"
                                    style="opacity:0.20000000298023224;isolation:isolate" />
                                <path
                                    d="M15.783,8.85H8.512V23.8h7.271a1.2,1.2,0,0,0,1.194-1.191V10.041A1.2,1.2,0,0,0,15.783,8.85Z"
                                    style="opacity:0.20000000298023224;isolation:isolate" />
                                <path
                                    d="M15.132,8.85H8.512V23.8h6.62a1.2,1.2,0,0,0,1.194-1.191V10.041A1.2,1.2,0,0,0,15.132,8.85Z"
                                    style="opacity:0.20000000298023224;isolation:isolate" />
                                <path
                                    d="M3.194,8.85H15.132a1.193,1.193,0,0,1,1.194,1.191V21.959a1.193,1.193,0,0,1-1.194,1.191H3.194A1.192,1.192,0,0,1,2,21.959V10.041A1.192,1.192,0,0,1,3.194,8.85Z"
                                    style="fill:url(#a)" />
                                <path
                                    d="M5.7,19.873l2.511-3.884-2.3-3.862H7.758L9.013,14.6c.116.234.2.408.238.524h.017c.082-.188.169-.369.26-.546l1.342-2.447h1.7l-2.359,3.84,2.419,3.905H10.821l-1.45-2.711A2.355,2.355,0,0,1,9.2,16.8H9.176a1.688,1.688,0,0,1-.168.351L7.515,19.873Z"
                                    style="fill:#fff" />
                                <path d="M28.806,3H19.581V9.5H30V4.191A1.192,1.192,0,0,0,28.806,3Z"
                                    style="fill:#33c481" />
                                <path d="M19.581,16H30v6.5H19.581Z" style="fill:#107c41" />
                            </svg>
                        </span>
                        Descargar Excel
                    </button>
        </form>

        @if (isset($resultados) && $resultados->count() > 0)
            <table>
                <thead>
                    <tr>
                        <th>Cliente</th>
                        <th>Fecha Cargue</th>
                        <th>Congelado</th>
                        <th>Facturado</th>
                        <th>Fecha Siigo</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($resultados as $row)
                        <tr>
                            <td>{{ $row->cliente }}</td>
                            <td>{{ $row->fecha_cargue }}</td>
                            <td>{{ $row->fecha_siigo ? 'Si' : 'No' }}</td>
                            <td>{{ $row->facturar ? 'Si' : 'No' }}</td>
                            <td>{{ $row->fecha_siigo }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        @endif

    </div>
</div>
</div>
</div>
</div>

<x-footer />
